...
export default async function FormularioPage({
  ...
  const { tipo = 'vehiculo' } = await searchParams
  const isVendedor = tipo === 'vendedor'

  return (
    <>
      {/* Header — hidden on print */}
      <div className="no-print flex items-center justify-between px-8 py-4 bg-white border-b flex-wrap gap-3" style={{ borderColor: 'var(--gray-line)' }}>
        <div className="flex items-center gap-1 rounded-pill p-1" style={{ background: 'var(--color-surface-alt)' }}>
          <Link
            href="/admin/formulario?tipo=vehiculo"
            className="px-4 py-1.5 rounded-pill text-[13px] font-[500] transition-colors"
            style={!isVendedor ? { background: '#012538', color: '#fff' } : { color: 'var(--text-muted)' }}
          >
            Vehículo
          </Link>
          <Link
            href="/admin/formulario?tipo=vendedor"
            className="px-4 py-1.5 rounded-pill text-[13px] font-[500] transition-colors"
            style={isVendedor ? { background: '#012538', color: '#fff' } : { color: 'var(--text-muted)' }}
          >
            Vendedor
      ...
      {isVendedor ? <SellerPrintForm /> : <VehiclePrintForm />}
    </>
  )
}

function VehiclePrintForm() {
  return (
  ...
  return (
    <div className="print-page p-10 max-w-[800px] mx-auto">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 28 }}>
        <div>
          <div style={{ fontSize: 22, fontWeight: 600, letterSpacing: '-0.02em', color: '#012538' }}>LoteCUU</div>
          <div style={{ fontSize: 11, color: '#6b7280', marginTop: 2 }}>Formulario de alta de vendedor</div>
        </div>
        <div style={{ textAlign: 'right', fontSize: 11, color: '#6b7280' }}>
          <div>Chihuahua, México</div>
          <div style={{ marginTop: 4 }}>
      ...
      {/* Identidad del lote */}
      <div style={{ marginBottom: 24 }}>
        <div className="section-title">Datos del lote / vendedor</div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px 24px' }}>
          <Field label="Nombre del vendedor (persona)" />
          <Field label="Nombre comercial del lote" />
        </div>
      </div>
      ...
      {/* Descripción */}
      <div style={{ marginBottom: 24 }}>
        <div className="section-title">Descripción del lote / negocio</div>
        {[0, 1, 2].map((i) => (
          <div key={i} className="field-line" style={{ marginBottom: 10 }} />
        ))}
      </div>
      ...
      {/* Cuenta de acceso */}
      <div style={{ marginBottom: 24 }}>
        <div className="section-title">Acceso al portal (opcional)</div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px 24px' }}>
          <Field label="¿Desea subir sus autos usted mismo?  □ Sí  □ No" />
          <Field label="Correo para su cuenta de acceso" />
        </div>
      </div>

      {/* Materiales */}
      <div style={{ marginBottom: 24 }}>
        <div className="section-title">Materiales proporcionados</div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
          <Checkbox label="Logo del lote" />
          <Checkbox label="Foto de perfil" />
          <Checkbox label="Inventario inicial de autos" />
        </div>
      </div>
